...
import { getProductOptions } from "../../react-queries/adminQueries";
import { useMutation, useQuery } from "@tanstack/react-query";
import { useParams, useNavigate } from 'react-router';
import { updateProductOptions } from '../../react-queries/adminQueries';
...
const EditProduct = () => {
  ...
  const {data, isPending, error, refetch} = useQuery(getProductOptions(id))
       ...
       >
               ...
               <label htmlFor="image">
                  {imagePreview ? (
                   ...
                   />
                      ...
                      src={`${import.meta.env.VITE_BACKEND_URL}/${data.product.image}`}
                     alt="Preview"
                     className="w-40 h-40 object-cover  rounded"
                   />)}
         ...
         <div className="mb-2 ">
           <label className="text-sm flex  items-center gap-2">
            ...
            <div>
              <button type="submit"  className={`bg-black text-white px-3 py-1.5 rounded hover:bg-neutral-800 cursor-pointer `}>Update product</button>
          </div>
